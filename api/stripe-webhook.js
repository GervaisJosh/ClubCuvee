...
var createRestaurant = async (data) => {
  ...
  const { data: restaurant, error } = await supabaseAdmin.from("restaurants").insert([data]).select().single();
  if (error) {
...
var updateRestaurantInvite = async (token, data) => {
  ...
  const { error } = await supabaseAdmin.from("restaurant_invitations").update(data).eq("token", token);
  if (error) {
